...
import pandas as pd
import plotly.graph_objects as go  # Usamos graph_objects en vez de plotly.express
import numpy as np
from wordcloud import WordCloud
import base64
...
app.title = "Dashboard de Accidentes"

# Layout con diseño organizado
app.layout = dbc.Container([
    html.H1("Dashboard de Accidentes de Tráfico", className="text-center my-4"),

    dbc.Row([
        dbc.Col([
            html.Label("Filtrar por condición climática:"),
            dcc.Dropdown(
                id="filtro_clima",
                options=[{"label": val, "value": val} for val in df_datos["weather_condition"].dropna().unique()],
                value=[],
                multi=True
            )
        ])
    ...
    dbc.Row([
        dbc.Col([
            html.H4("OBJETIVO 1: Condición climática más incidente"),
            dcc.Graph(id="grafico_clima")
        ], md=6),
        dbc.Col([
            html.H4("OBJETIVO 2: Horario del accidente"),
            dcc.Graph(id="grafico_hora")
        ], md=6)
    ]),

    dbc.Row([
        dbc.Col([
            html.H4("OBJETIVO 3: Influencia de la luz del día"),
            dcc.Graph(id="grafico_luz")
        ])
    ]),

    dbc.Row([
        dbc.Col([
            html.H4("OBJETIVO 4: Wordcloud del tipo de choque"),
            html.Img(id="wordcloud_img", style={'width': '100%', 'height': 'auto'})
        ], md=5)
    ]),

    dbc.Row([
        dbc.Col([
            html.H4("OBJETIVO 5: Accidentes por mes"),
            dcc.Graph(id="grafico_mensual")
        ], md=6),
        dbc.Col([
            html.H4("OBJETIVO 6: Accidentes totales por año"),
            dcc.Graph(id="grafico_anual")
        ], md=6)
    ])
], fluid=True)
...
@app.callback(
    ...
    Output("grafico_anual", "figure"),
    Input("filtro_clima", "value")
)
def actualizar_graficos(filtro_clima):
    df_datos_filtrado = df_datos.copy()
    if filtro_clima:
        df_datos_filtrado = df_datos_filtrado[df_datos_filtrado["weather_condition"].isin(filtro_clima)]

    # Crear el histograma para clima
    fig_clima = go.Figure(go.Histogram(x=df_datos_filtrado['weather_condition']))
    fig_clima.update_layout(
        title='Frecuencia Condición Climática',
        xaxis_title='Condición Climática',  # Título eje X
        yaxis_title='Frecuencia'  # Título eje Y
    )

    # Crear el histograma para hora, con separación de barras
    fig_hora = go.Figure(go.Histogram(x=df_datos_filtrado['crash_hour'], nbinsx=24))
    fig_hora.update_layout(
        title='Accidentes por Hora',
        xaxis_title='Hora del Accidente',  # Título eje X
        yaxis_title='Número de Accidentes',  # Título eje Y
        bargap=0.5  # Esto separa las barras
    )

    # Gráfico de pie para la condición de iluminación
    lighting_counts = df_datos_filtrado['lighting_condition'].value_counts()
    total = lighting_counts.sum()
    grouped = lighting_counts[lighting_counts / total >= 0.03]
    grouped["OTROS"] = lighting_counts[lighting_counts / total < 0.03].sum()
    fig_luz = go.Figure(go.Pie(labels=grouped.index, values=grouped.values))
    fig_luz.update_layout(title='Condición de Iluminación')

    # Wordcloud de tipo de choque
    text = ' '.join(df_datos_filtrado['first_crash_type'].dropna().astype(str))
    wordcloud = WordCloud(width=800, height=400, background_color='white', collocations=False).generate(text)
    img = BytesIO()
    ...
    # Gráfico mensual
    mensual = df_datos_filtrado[['crash_month', 'injuries_incapacitating', 'injuries_non_incapacitating']]
    mensual = mensual.groupby('crash_month')[['injuries_incapacitating', 'injuries_non_incapacitating']].sum()

    # Usamos go.Scatter con fill='tozeroy' para crear un gráfico de área
    fig_mensual = go.Figure()
    fig_mensual.add_trace(go.Scatter(
        x=mensual.index,
        y=mensual['injuries_incapacitating'],
        mode='lines',
        name='Incapacitating Injuries',
        fill='tozeroy'  # Esto crea el área bajo la línea
    ))
    fig_mensual.add_trace(go.Scatter(
        x=mensual.index,
        y=mensual['injuries_non_incapacitating'],
        mode='lines',
        name='Non-Incapacitating Injuries',
        fill='tonexty'  # Esto rellena el área entre las líneas
    ))
    fig_mensual.update_layout(
        title='Accidentes Incapacitantes vs No Incapacitantes por Mes',
        xaxis_title='Mes',  # Título eje X
        yaxis_title='Número de Accidentes'  # Título eje Y
    )

    # Gráfico anual
    df_datos_filtrado['year'] = pd.to_datetime(df_datos_filtrado['crash_date']).dt.year
    df_totals = df_datos_filtrado.groupby('year')['injuries_total'].sum().reset_index()
    fig_anual = go.Figure(go.Scatter(
        x=df_totals['year'], y=df_totals['injuries_total'], mode='lines+markers', name='Accidentes Totales'
    ))
    fig_anual.update_layout(
        title='Accidentes por Año',
        xaxis_title='Año',  # Título eje X
        yaxis_title='Total de Accidentes'  # Título eje Y
    )

    return fig_clima, fig_hora, fig_luz, wordcloud_src, fig_mensual, fig_anual
...
# Agregar el token de ngrok (hazlo antes de usar ngrok.connect)
!ngrok config add-authtoken 2vbJ4QcAxTffZPcgSbHc6K5xgne_7CN49fscLisDLkdJZeDM2
# Crear función para correr Dash
def run_dash():
    app.run(port=8050)
